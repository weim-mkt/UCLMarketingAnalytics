...
-   Understand keys concepts of causal inference and Rubin's potential outcome framework
...
To test the effectiveness of a new COVID vaccine, the government sends invites to the general public to seek volunteers for the clinical trial. The voluntarily recruited subjects then received the vaccination.

After 3 months, the government runs a t-test on the infection rate between general public who didn't receive the vaccine versus volunteers who received the vaccine, and finds that the infection rate is statistically indifferent.
...
-   On rainy days, we observe more umbrellas on the street
    ...
    -   incorrect causal statement: more umbrellas causes more rainfall
...
## Conceptualization of Causal Inference for Business Managers
...
For a random customer Tony Stark,

-   In universe 1, Tom adopts the LP, and Tony has a retention rate of 70%
-   In universe 2, Tom does not adopt the LP, and Tony has a retention rate 60%.

```{r}
#| echo: false
pacman::p_load(data.table,knitr,kableExtra)

kable(data.frame(subject = c("Tony"), 
           Yt = 0.7,
           Yc = 0.6,
           `Yt-Yc` = 0.1,
           check.names = F),
...
-   Conclusion: The treatment effect of LP on Tony is +10%.
...
```{r}
#| echo: false
kable(data.frame(subject = c("Tony","Captain","Thor","Hulk"), 
...
kable(data.frame(subject = c("Tony","Captain","Thor","Hulk"), 
           ...
           `Yt-Yc` = c("?","?","?","?"),
           check.names = F),
      col.names = c("Subject", "$Y_t$","$Y_c$","$Y_t - Y_c$"),
      escape = F) %>%
  kable_styling(font_size = 8, bootstrap_options = c("striped", "hover"))
```
...
-   Individual treatment is never observed, and individual treatment effects are likely to vary with individual customers
...
-   Sometimes, we may also care about the treatment effects on the treated (ATT)
    ...
    -   e.g., NHS only gives booster jabs to vulnerable group

$$
ATT = E[\delta_i|D_i = 1] = E[Y^1_i - Y^0_i|D_i = 1]
$$

-   Similarly, we can also define the Treatment Effect on the Untreated (ATU), but ATU is often of less policy interest.
...
-   If the treatment allocation is random among all individuals, the above gives ATE, because the first term is an estimate of the ATT and ATT = ATE under randomization; the second term has an expected value of zero.
