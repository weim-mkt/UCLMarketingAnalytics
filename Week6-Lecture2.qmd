...
-   $Y$ the **outcome** variable; $X = (X_1,X_2,...,X_p)$ are a set of **explanatory variables**.
-   $\theta$ represents the set of **function** **parameters** of the DGP $f$.
-   $\epsilon$ is the random **error term**, with mean zero.

## Conditional Mean

- In causal inference, we care about the conditional mean of the outcome variable, denoted as $E[Y|X=x]$.
    
- For example, in an RCT, Y is the outcome variable, X is whether or not customers receive the treatment, then from the basic identity of causal inference, we have
$$
ATE = E[Y^1 - Y^0] = E[Y^1|D=1] - E[Y^0|D=0]
$$

- However, since we never know the true DGP, we need to come up with a **model** that approximates the $E[Y|X=x]$.

- Why not start with something simple? What would be the simplest possible functional relationship for $E[Y|X=x]$?

## Linear Regression Models

- If we assume a **linear**, **additive** relationship for $Y = f(X;\theta) + \epsilon$, we have a simple linear regression model, as follows,
...
-   $\epsilon_i$: error term/disturbance

    -   Error term will have the expected mean of 0
...
-   In this module, we will be using the `lfe` package, because it's able to estimate high-dimensional fixed effects.

```{r echo=TRUE}

pacman::p_load(modelsummary,lfe)

OLS_result <- felm( 
   formula = total_spending ~ Income, # Y ~ X
...
-   For each guess of a and b, we can compute the error of for customer $i$, $$
    e_i = y_{i}-a-b x_{1i}
    $$
...
#| echo: false
felm(data = data_full,
     formula = total_spending ~ Income + Kidhome) %>%
...
-   Due to randomness of the error term, all coefficients estimates follow a $t$ distribution, with the mean being the true coefficient value, and the standard deviation being the standard error we estimated.
