...
Which means, only if a customer responds to us with at least the break-even response rate can we recover the costs of making an marketing offer.
...
data_test <- data_test %>%
  mutate(predicted_prob_randomforest = )
...
data_test <- data_test %>%
  mutate(is_target_randomforest = )

# total marketing costs
total_costs_of_mailing_randomforest <- cost_per_offer * sum()

# total profits from responding customers
total_profit_randomforest <- sum((data_test%>%
                                    filter())$Response) * profit_per_customer
