...
title: "Class 7 Supervised Learning for STP (II): Supervised Learning"
df-print: kable
colorlinks: true
...
1.  First, we find that `cyl` matters more than `hp` in terms of predicting mpg, so we split two branches according to `cyl`. We find that {4} and {6,8} split can best differentiate car models.
    -   `cyl` = 4, go to the left branch

    -   `cyl` = 6 or 8, go to the right branch
...
3.  For car models with more than 5 cylinders, we find that `hp` does not differentiate car models, so we do not further split.
