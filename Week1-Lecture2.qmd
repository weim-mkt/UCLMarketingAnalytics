...
$$
\mathrm{CLV} =\sum_{t=1}^{N} \frac{\left(M_{t}-c_{t}\right) r^{(t-1)}}{(1+d)^{t}} - CAC
$$
...
-   $d$ is the rate for discounting future cash flows
...
$$
C L V_{N} = \sum_{t=1}^{N} \frac{g r^{(t-1)}}{(1+d)^{t}} => C L V_{N}=\mathrm{g} \cdot \frac{1-\left(\frac{r}{1+d}\right)^{N}}{1+d-r}  => 
C L V_{\infty}=\frac{g}{(1+d-r)}
$$
