...
-   Load Tesco dataset and necessary packages
...
-   `Week_of_Year` should be treated as a categorical variables, because seasonality should not be linear in time.
...
-   Run the above multivariate regression in R, where the dependent variable is `Units_Sold` and the RHS variables are `Unit_Price` and the others.
...
-   **Exercise**: Interpret the coefficients in this regression table?
...
[^1]: You will learn how to use optimize solver in term 2 Operations Analytics.
